...
public class Ts1dia extends JFrame implements ActionListener {
				...
				if (a > 0 && a <= 5 && b >= 0 && b <= 200) {
					...
					if (a <= TsSc.d) {
						...
						try {
							...
							PreparedStatement ps = con.prepareStatement("update ts1 set seat=? where Seat_type=?");
							ps.setInt(1, rs);
						...
						Ts1Atmd ts1atd = new Ts1Atmd() ;
						super.dispose();

					}
					if (a > TsSc.d) {
						JOptionPane.showMessageDialog(this, "Seat not available only " + TsSc.d + " seat left");
