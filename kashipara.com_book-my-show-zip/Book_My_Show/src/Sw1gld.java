...
public class Sw1gld extends JFrame implements ActionListener {
				...
				if (a > 0 && a <= 20 && b >= 0 && b <= 200) {
					...
					if (a <= SwSc.g) {
						...
						try {
							...
							PreparedStatement ps = con.prepareStatement("update sw1 set seat=? where Seat_type=?");
							ps.setInt(1, rs);
							ps.setString(2, "gold");
							ps.executeUpdate();
						...
						Sw1Atmg sw1atg = new Sw1Atmg();
						super.dispose();

					}
					if (a > SwSc.g) {
						JOptionPane.showMessageDialog(this, "Seat not available only " + SwSc.g + " seat left");
