...
public class Ts1gld extends JFrame implements ActionListener {
			...
			if (e.getSource() == b2) {
				...
				if (a > 0 && a <= 20 && b >= 0 && b <= 200) {
					...
					if (a <= TsSc.g) {
						...
						Ts1Atmg ts1atg = new Ts1Atmg();
						super.dispose();

					}
					if (a > TsSc.g) {
						JOptionPane.showMessageDialog(this, "Seat not available only " + TsSc.g + " seat left");
