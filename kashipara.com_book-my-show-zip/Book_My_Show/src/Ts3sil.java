...
public class Ts3sil extends JFrame implements ActionListener {
		...
		if (e.getSource() == b2) {
			...
			if (a > 0 && a <= 200 && b >= 0 && b <= 200) {
				...
				if (a <= TsSc3.s) {
					...
					Ts3Atms ts3ats =new Ts3Atms();
					super.dispose();

				}
				if (a > TsSc3.s){
					JOptionPane.showMessageDialog
					(this, "Seat not available only " +TsSc3.s +" seat left");
