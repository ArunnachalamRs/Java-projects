...
public class Sw2gld extends JFrame implements ActionListener {
	JLabel l1, l2, l3, l4;
	float cost = 0;
	float tc = 0, sc = 0;
			...
			if (e.getSource() == b2) {
				...
				if (a > 0 && a <= 20 && b >= 0 && b <= 200) {
					...
					if (a <= SwSc2.g) {
						...
						Sw2Atmg sw2atg = new Sw2Atmg();
						super.dispose();

					}
					if (a > SwSc2.g) {
						JOptionPane.showMessageDialog(this, "Seat not available only " + SwSc2.g + " seat left");
