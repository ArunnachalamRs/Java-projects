...
public class Ts3gld extends JFrame implements ActionListener {
				...
				if (a > 0 && a <= 20 && b >= 0 && b <= 200) {
					...
					if (a <= TsSc3.g) {
						...
						try {
							...
							PreparedStatement ps = con.prepareStatement("update ts3 set seat=? where Seat_type=?");
							ps.setInt(1, rs);
							ps.setString(2, "gold");
							ps.executeUpdate();
						...
						Ts3Atmg ts3atg =new Ts3Atmg();
						super.dispose();

					}
					if (a > TsSc3.g) {
						JOptionPane.showMessageDialog(this, "Seat not available only " + TsSc3.g + " seat left");
