...
public class Shad2gld extends JFrame implements ActionListener {
	JLabel l1, l2, l3, l4;
	float cost = 0;
	float tc = 0, sc = 0;
	...
	public Shad2gld() {
		...
		l1 = new JLabel("Shadi Mei Zor Ana");
			...
			if (e.getSource() == b2) {
				...
				if (a > 0 && a <= 20 && b >= 0 && b <= 200) {
					...
					if (a <= ShadSc2.g) {
						...
						Shad2Atmg shad2atg = new Shad2Atmg() ;
						super.dispose();

					}
					if (a > ShadSc2.g) {
						JOptionPane.showMessageDialog(this, "Seat not available only " + ShadSc2.g + " seat left");
