...
public class Ts2gld extends JFrame implements ActionListener {
	JLabel l1, l2, l3, l4;
	float cost = 0;
	float tc = 0, sc = 0;
				...
				if (a > 0 && a <= 20 && b >= 0 && b <= 200) {
					...
					if (a <= TsSc2.g) {
						...
						try {
							...
							PreparedStatement ps = con.prepareStatement("update ts2 set seat=? where Seat_type=?");
							ps.setInt(1, rs);
							ps.setString(2, "gold");
							ps.executeUpdate();
						...
						Ts2Atmg ts2atg =new Ts2Atmg();						
						super.dispose();

					}
					if (a > TsSc2.g) {
						JOptionPane.showMessageDialog(this, "Seat not available only " + TsSc2.g + " seat left");
