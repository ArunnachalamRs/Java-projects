...
public class Ts2dia extends JFrame implements ActionListener {
	JLabel l1, l2, l3, l4 ;
	float cost = 0;
	float tc = 0, sc = 0;
			...
			if (e.getSource() == b2) {
				...
				if (a > 0 && a <= 5 && b >= 0 && b <= 200) {
					...
					if (a <= TsSc2.d) {
						...
						Ts2Atmd ts2atd = new Ts2Atmd();
						super.dispose();

					}
					if (a > TsSc2.d) {
						JOptionPane.showMessageDialog(this, "Seat not available only " + TsSc2.d + " seat left");
